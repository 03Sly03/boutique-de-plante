        ...
        <div className='bdpc-cart'>
            <h2>Panier</h2>
            {
                cart.map(({ name, price, amount }, index) => (
                    <div key={`${name}-${index}`}>
                        <p><ImCross onClick={() => handleItemCartDelete(name)} style={{ cursor: 'pointer' }} /> {name} {price}€ <AiOutlineMinusCircle id="minus" style={{ cursor: 'pointer' }} onClick={(e) => substracToCart(name, price)} /> {amount} <AiOutlinePlusCircle id="plus" style={{ cursor: 'pointer' }} onClick={(e) => addToCart(name, price)} /></p>
                    </div>
                ))
            }
